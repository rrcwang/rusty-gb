...
impl Cpu {
    ...
    fn alu_add_words(&mut self, x: u16, y: u16) -> u16 {
        unimplemented!("Word length arithmetic.");
        0x0000
    }

    /// TODO:
    fn alu_sub_words(&mut self, x: u16, y: u16) -> u16 {
        unimplemented!("Word length arithmetic.");
        0x0000
    }
    ...
    fn alu_dec_byte(&mut self, x: u8) -> u8 {
        let result = x.wrapping_sub(x);

        self.registers.set_flag(Flag::Z, result == 0);
        self.registers.set_flag(Flag::N, true);
        self.registers.set_flag(Flag::H, (x & 0x0F) == 0);
        // 0x*0 - 0x01 = 0x(*-1)F. need set half-carry
        // 0x*0 & 0x0F = 0
        result
    }
    ...
    fn execute_instr(&mut self, instruction: u8) -> u8 {
        match instruction {
            ...
            0x02 => {
                ...
                let address = self.registers.get_16b_reg(Register16b::BC);
            ...
            0x03 => {
                ...
                let value = self.registers.get_16b_reg(Register16b::BC);
                self.registers
                    .set_16b_reg(Register16b::BC, value.wrapping_add(1));
                8
            }
            0x04 => {
                // TOTEST
                // INC B
                let mut value = self.registers.get_8b_reg(Register8b::B);
            ...
            0x07 => {
                ...
                self.registers.set_8b_reg(Register8b::F, 0b_0000_0000);
            ...
            0x09 => {
                ...
                self.unimpl_instr();
                let bc = self.registers.get_16b_reg(Register16b::BC);
                let hl = self.registers.get_16b_reg(Register16b::HL);
            ...
            0x0C => {
                ...
                // INC C
                self.unimpl_instr();
                let value = self.registers.get_8b_reg(Register8b::B);
